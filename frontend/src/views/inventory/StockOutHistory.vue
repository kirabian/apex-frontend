...
import {
    Search, ArrowLeft, RefreshCw, Box, Calendar, User, Truck, ClipboardList, Info
...
const fetchData = async (page = 1) => {
    ...
    try {
        ...
        const response = await stockOut.list(params);
        ...
        <div class="bg-surface-800 rounded-2xl border border-surface-700 p-4">
            <div class="flex flex-col sm:flex-row gap-4 justify-between">
                <div class="relative w-full sm:w-72">
                    ...
                    <input v-model="searchQuery" type="text" placeholder="Cari ID, Penerima, atau..."
                        ...
                        <tr v-for="item in items" :key="item.id" class="hover:bg-surface-700/30 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    <span class="font-mono text-xs text-primary-400">{{ item.transaction_code }}</span>
                            ...
                            <td class="px-6 py-4 max-w-xs truncate text-text-secondary">
                                {{ item.note || '-' }}
                            </td>
                            <td class="px-6 py-4">
